...
public class RECISTCalculator {
	...
	private static final Logger logger = Logger.getLogger("RECISTCalculator");
	...
	public static ImageAnnotation[][] loadAndSortAIMFilesByStudyDate(
			List<ImageAnnotation> imageAnnotations, String server) {
	...
	public static CalculationResult calculateRECIST(
		...
		for (int i = 0; i < metrics.length; i++)
			tumorAnalysisCalculators[i] = new TumorAnalysisCalculator(
					imageAnnotationsByStudyDate, metrics[i], df);
		...
		for (int i = 0; i < lesions.length; i++) {
			...
			if (ia.getNumberOfAnatomicEntityCollections() > 0) {
				AnatomicEntityCollection aec = ia
						.getAnatomicEntityCollection(0);
