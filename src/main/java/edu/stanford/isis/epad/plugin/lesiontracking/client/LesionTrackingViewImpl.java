...
//        seriesListBox.setMultipleSelect(false);
    ...
    /*
    @UiHandler("studiesListBox")
    public void onStudySelected(ChangeEvent changeEvent)
    {
        int selectedIndex = studiesListBox.getSelectedIndex();
        if(selectedIndex != -1)
            lesionTracking.onStudySelected(studiesListBox.getItemText(selectedIndex));
    }
     */
    /*
    @UiHandler("seriesListBox")
    public void onSeriesSelected(ChangeEvent changeEvent)
    {
        int selectedIndex = seriesListBox.getSelectedIndex();
        if(selectedIndex != -1)
            lesionTracking.onSeriesSelected(seriesListBox.getItemText(selectedIndex));
    }
    */
    ...
    */
    /*
    public void loadSeriesList(List<String> seriess)
    {
        seriesListBox.clear();
        for(String series : seriess)
            seriesListBox.addItem(series);
    }
    */
    ...
    public void loadRECISTTable(String[][] recistTable)
    {
        ...
        {
            int rowIndex = recistFlexTable.insertRow(recistFlexTable.getRowCount());
    ...
    public void setNumberTimePointsInStudy(String metricUnits, int numberOfTimePoints)
    {
        recistFlexTable.removeAllRows();
        recistFlexTable.insertRow(recistFlexTable.getRowCount());

        recistFlexTable.setWidget(0, 0, new RECISTTableTitleRowLabel("Lesion ID"));
        recistFlexTable.setWidget(0, 1, new RECISTTableTitleRowLabel("Location"));
        recistFlexTable.setWidget(0, 2, new RECISTTableTitleRowLabel("Baseline (" + metricUnits + ")"));

        for(int i = 0; i < numberOfTimePoints - 1; i++)
            recistFlexTable.setWidget(0, 3 + i, new RECISTTableTitleRowLabel("Follow Up " + (i+1) + " (" + metricUnits + ")"));
    }
    ...
    public void addLesion(Lesion lesion)
    {
        ...
        Label lesionIDLabel 	  = new RECISTTableVariableEntryLabel(lesion.getLesionID(), row);
        Label lesionLocationLabel = new RECISTTableVariableEntryLabel(lesion.getLocation(), row);
        ...
        {
            RECISTTableVariableEntryLabel label = new RECISTTableVariableEntryLabel(lesion.getMeasurementsForMetric(metrics.get(0)).get(i), row);
            label.addClickHandler(new LesionClickHandler(lesion, i));
            recistFlexTable.setWidget(recistFlexTable.getRowCount() - 1, i+2, label);
        }
    }

    public void addResponseRows(String metric, String metricUnits, String[] metricSums, String[] responseRates, String[] responseRatesSinceBaseline, String[] responseRatesSinceNadir, String[] responseCategories)
    {
        int metricNameRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());
        int sumOfMetricsRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());
//        int responseRateRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());
        int responseRateSinceBaselineRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());
        int responseRateSinceNadirRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());
        int responseCategoryRow = recistFlexTable.insertRow(recistFlexTable.getRowCount());

        // Metric name row.
        recistFlexTable.setWidget(metricNameRow, 0, new RECISTTableTitleRowLabel(metric));
        recistFlexTable.getFlexCellFormatter().setColSpan(metricNameRow, 0, 5);

        // The sum of metrics row.
        recistFlexTable.setWidget(sumOfMetricsRow, 0, new RECISTTableResponseRowLabel("Sum of " + metric + "s (" + metricUnits + ")"));
        recistFlexTable.getFlexCellFormatter().setColSpan(sumOfMetricsRow, 0, 2);

        for(int i = 0; i < metricSums.length; i++)
            recistFlexTable.setWidget(sumOfMetricsRow, i + 1, new RECISTTableTitleRowLabel(metricSums[i]));

        // The response rate row title.
//        recistFlexTable.setWidget(responseRateRow, 0, new RECISTTableResponseRowLabel("Response Rate (%)"));
//        recistFlexTable.getFlexCellFormatter().setColSpan(responseRateRow, 0, 2);

        // The response rate since baseline title.
        recistFlexTable.setWidget(responseRateSinceBaselineRow, 0, new RECISTTableResponseRowLabel("Change from Baseline (%)"));
        recistFlexTable.getFlexCellFormatter().setColSpan(responseRateSinceBaselineRow, 0, 2);

        // The response rate since nadir title.
        recistFlexTable.setWidget(responseRateSinceNadirRow, 0, new RECISTTableResponseRowLabel("Change from Nadir (%)"));
        recistFlexTable.getFlexCellFormatter().setColSpan(responseRateSinceNadirRow, 0, 2);

        // The response category row title.
        recistFlexTable.setWidget(responseCategoryRow, 0, new RECISTTableResponseRowLabel("Target Lesion Response Category"));
        recistFlexTable.getFlexCellFormatter().setColSpan(responseCategoryRow, 0, 2);

        for(int i = 1; i < responseRates.length; i++)
        {
//            recistFlexTable.setWidget(responseRateRow, i + 1, new RECISTTableTitleRowLabel(responseRates[i]));
            recistFlexTable.setWidget(responseRateSinceBaselineRow, i + 1, new RECISTTableTitleRowLabel(responseRatesSinceBaseline[i]));
            recistFlexTable.setWidget(responseRateSinceNadirRow, i + 1, new RECISTTableTitleRowLabel(responseRatesSinceNadir[i]));
            recistFlexTable.setWidget(responseCategoryRow, i + 1, new RECISTTableTitleRowLabel(responseCategories[i]));
        }
    }
    ...
    private class LesionIDClickHandler implements ClickHandler
    {
        ...
        {
            RECISTTableVariableEntryLabel recistTableVariableEntryLabel = (RECISTTableVariableEntryLabel)clickEvent.getSource();
            ...
            for(int i = 1; i < recistFlexTable.getRowCount() - 2; i++)
                for(int j = 2; j < recistFlexTable.getCellCount(i); j++)
                {
                    ...
                    if(label instanceof RECISTTableVariableEntryLabel)
                    {
                        recistTableVariableEntryLabel = (RECISTTableVariableEntryLabel)label;
                        if(recistTableVariableEntryLabel != null)
                            recistTableVariableEntryLabel.setSelected(i == row && j == temporalIndex + 2);
                    }
                }
    ...
    public void displayRadiologyImages(String[] radiologyImageURLs)
    {
        ...
        radiologyImagesFlexTable.insertRow(radiologyImagesFlexTable.getRowCount());
        int colIndex = 0;
