...
public class RECISTTableServlet extends HttpServlet
{
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        File file = downloadRECISTTableImage(cr, getServletContext().getRealPath("/"));
    ...
    public static File downloadRECISTTableImage(CalculationResult cr, String realPath)
    {
        ...
        BufferedImage image = new BufferedImage(colSize * (cr.getNumberOfTimePointsInStudy() + 2), rowSize * metrics.length * (8 + cr.getLesions().length), BufferedImage.TYPE_3BYTE_BGR);
        ...
        for(int i = 0; i < metrics.length; i++)
        {
            ...
            g.drawString("Lesion ID", titleColumnOffset + colSize, currentRow * rowSize);
            g.drawString("Baseline", titleColumnOffset + 2 * colSize, currentRow * rowSize);
            g.drawString("Follow Up 1", titleColumnOffset + 3 * colSize, currentRow * rowSize);
            g.drawString("Follow Up 2",  titleColumnOffset + 4 * colSize, currentRow * rowSize);
            ...
            for(int j = 0; j < lesions.length; j++)
            {
                ...
                g.drawString(lesion.getLesionID(), titleColumnOffset + colSize, currentRow * rowSize);

                List<String> metricValues = lesion.getMeasurementsForMetric(metrics[i]);
                for(int k = 0; k < metricValues.size(); k++)
                {
                    String measurement = metricValues.get(k);
                    g.drawString(measurement, titleColumnOffset + (k+2) * colSize, currentRow * rowSize);
                }
            ...
            currentRow++;
            g.drawString(metrics[i] + " (" + metricUnits[i] + ")", 0, rowSize * currentRow);
            for(int j = 0; j < cr.getNumberOfTimePointsInStudy(); j++)
            {
                String value = cr.getMetricSums()[i][j];
                if(value == null || "N/A".equals(value))
                    value = "";
                g.drawString(value, titleColumnOffset + colSize + j * colSize, rowSize * currentRow);
            }
            ...
            currentRow++;
            g.drawString("Target Lesion Response Category", 0, rowSize * currentRow);
            for(int j = 0; j < cr.getNumberOfTimePointsInStudy(); j++)
            {
        ...
        try {
            ...
            return writeImage(image, realPath + "recist_table_" + (int)Math.ceil(Math.random() * 10000) + ".jpg");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static File writeImage(BufferedImage image, String fileName) throws IOException
    {
