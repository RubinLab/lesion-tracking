...
public class RECISTCalculator
{
    ...
    public static CalculationResult calculateRECIST(ImageAnnotation[][] imageAnnotationsByStudyDate, String[] metrics)
    {
        /*
        final String servletPath = getServletContext().getRealPath("");

        ImageAnnotation[][] imageAnnotationsByStudyDate = AIMFileReader.loadAndSortAIMFilesByStudyDate(aimFilenames);
*/
        ...
        for(int i = 0; i < metrics.length; i++)
            tumorAnalysisCalculators[i] = new TumorAnalysisCalculator(imageAnnotationsByStudyDate, metrics[i]);
        ...
        for(int i = 0; i < lesions.length; i++)
        {
            ...
            String loc = ia.getAnatomicEntityCollection(0).getAnatomicEntity(0).getCodeMeaning();
